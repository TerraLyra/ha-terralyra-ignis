...
await page.getByRole('combobox').selectOption('all');assert.equal(await page.locator('.entry').count(),3);await page.getByRole('button',{name:/Lakástűz/}).click();assert.equal(await page.locator('a').count(),0);
