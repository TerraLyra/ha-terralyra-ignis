...
| --- | --- | --- |
| Victoria CFA-listed developer JSON | Product-specific developer conditions unresolved; offline research complete, runtime absent | [VicEmergency form](https://support.emergency.vic.gov.au/hc/en-gb/requests/new). Sending/approval is not confirmed in this record. |
| WA DFES-066 Incident Points | Explicit SLIP registration and dataset approval; catalogue lists CC BY 4.0 plus EmergencyWA terms | Start access request; contact **statepublicinfo@dfes.wa.gov.au**. Confirm public HACS/per-user authentication eligibility, including resource-level Government Use Only labels. |
| Tasmania TasALERT feeds | Explicit permission required; do not inherit legacy TFS feed terms | Request access at **info@alert.tas.gov.au** before adapter work. |
| SA CFS / ESO feeds | Original SA CKAN package reports CC BY-NC-ND 4.0; the national mirror omits the licence | Clarify local HACS use and current terms with CFS before implementation; commercial use needs separate permission. See [SA review](SOUTH_AUSTRALIA_ADAPTER_READINESS.md). |
